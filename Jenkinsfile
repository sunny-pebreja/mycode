...
pipeline {
    ...
    stages {
        ...
        stage('Cloning Git') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: "${params.BRANCH}"]], extensions: [], userRemoteConfigs: [[credentialsId: 'git-access-by-jenkins', url: 'https://gitlab.com/simublade/used-engine-admin.git']]])
            }
        }
